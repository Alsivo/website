...
"""
    ...
    common_rules += """
- 投稿本文にアルとシーボを登場させず、二人の名前や会話を書かない
- 「○○に悩んでいませんか？」に相当する自然な問題提起から始める
- 対象サービスを使うと何ができ、悩みの解消へどう近づけるかを短く伝える
- 記事で整理している内容を示し、「詳しくは記事で」という自然な誘導で締める
...
構成：
1. 読者の悩みを、普段使う言葉で一文にする
2. 「対象サービス名なら」で始め、それを使うと何ができるかを丁寧な一文にする
3. 「詳細は記事へ↓」でURLへつなぐ

条件：
- 日本語
- URLと#PRを除いて55〜80文字程度を目安
...
- 誇張しない
- 記事内容と一致させる
- 記事の章立て、確認項目、比較項目、設定方法などを具体的に列挙しない
- 「費用感」「確認項目」「決めやすく」「始めやすく」のような曖昧で硬い表現を使わない
- 文字数を減らすために助詞や語尾を削らず、必ず文として完結させる
- 1文目は自然な常体でよいが、2文目は「〜なら、〜できて、〜しやすくなります」のような自然な敬語にする
- 「〜で、〜ができ、〜なる」のように助詞を省略して機械的につながない
- 良い例：議事録作成に時間がかかって大変。Nottaなら、文字起こしやZoomとの連携ができて、議事録をまとめやすくなります。詳細は記事へ↓
- URLを最後にそのまま付ける
- ハッシュタグは0〜2個
