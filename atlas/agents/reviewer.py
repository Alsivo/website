...
def review_article(
    ...
    response = client.responses.create(
        ...
        instructions=(
            ...
            "本文のMarkdown階層も確認してください。"
            "短い要点一覧では、"
            "『- **項目名：** 説明文』の形式になっているか確認してください。"
            "項目名とコロンだけが太字で、説明文まで太字になっていないことを確認してください。"
            "1項目の説明が複数文にわたる詳細小項目では、"
            "項目タイトルが独立した太字行になり、"
            "空行の後に説明文が通常文字で記述されているか確認してください。"
            "操作手順や判断フローなど順序に意味がある場合だけ、"
            "番号付きリストを使用しているか確認してください。"
            "番号付き項目でもタイトルだけが太字で、"
            "説明文全体が太字になっていないことを確認してください。"
            "『項目タイトル：長い説明文』が1行に混在していて読みづらい場合は、"
            "短い要点一覧形式または詳細小項目形式へ整理するよう修正指示してください。"
            "「この記事でわかること」「特徴」「向いている人」「料金」「注意点」などの"
            "短い要点一覧も同じ基準で確認してください。"
            "文章の意味や事実内容を変える必要がなく、Markdown階層だけが問題の場合は、"
            "内容変更ではなく書式整理として改善指示してください。"
            "Markdown装飾記号が不要にエスケープされていないか確認してください。"
            "太字にする箇所が『\\*\\*項目名\\*\\*』のように"
            "バックスラッシュ付きで記述されている場合は問題として指摘してください。"
            "太字は『**項目名**』の形式に修正するよう指示してください。"
            "Markdownとして表示するアスタリスクの直前に"
            "不要なバックスラッシュが残っていないか確認してください。"
