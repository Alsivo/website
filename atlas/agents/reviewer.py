...
def review_article(
    ...
    response = client.responses.create(
        ...
        instructions=(
            ...
            "本文内の小項目タイトルと説明文のMarkdown構造も確認してください。"
            "小項目タイトルと説明文が同じ行に混在している場合は、"
            "項目タイトルをMarkdownの太字にし、"
            "空行を挟んで説明文を通常文字の別段落に分けるよう修正指示してください。"
            "説明文全体が太字になっている場合も修正を指示してください。"
            "順序に意味がない小項目へ不要な番号が付いている場合は指摘してください。"
            "操作手順など順序に意味がある場合だけ番号付きリストを許可してください。"
            "番号付き手順では、"
            "『1. **項目タイトル：**』の後に空行を入れ、"
            "説明文を通常文字で記述する構造になっているか確認してください。"
            "『1) 項目タイトル：説明文』のように、"
            "項目タイトルと説明文を1行にまとめた形式は修正対象としてください。"
